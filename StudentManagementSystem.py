...
def delete_student():
    try:
        ...
        messagebox.showinfo('Record','At least one record slected')
    ...
    def add_data():
        if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='':
            ...
            try:
                ...
                con.commit()
                result=messagebox.askyesno('Confire','Dta added successfully. Do you want to clean the form?',parent=add_window)
                if result:
                 ...
                 messagebox.showerror('Error','Id already Exit and enter new id no',parent=add_window)
                 return
            query='select * from student'
            mycursor.execute(query)
            fetched_data=mycursor.fetchall()
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute(query)
            query='create table student(id int not null primary key,name varchar(50),mobile varchar(10),email varchar(30),address varchar(100),gender varchar(20),dob varchar(20),date varchar(50),time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
